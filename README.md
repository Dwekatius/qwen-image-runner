...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/status-v1.3.0-success" alt="Status">
</p>
...
## Quick start (clone → install → first image)
...
Launch.bat      REM start the app (or use the "Qwen Image Runner" shortcut)
                REM first run: accept licences, pick a backend, download models (~14.7 GB), start the engine
                REM then type a prompt in Chat and press Send
```

New to all of this? **[docs/GUIDE.md](docs/GUIDE.md)** takes you step by step from the clone to your
first image, then explains every setting, the DeepSeek prompt assistant, the model files,
troubleshooting and the FAQ. Prerequisites, the manual install route and per-file checksums:
...
| Disk | ~25 GB free (engine ≈ 0.9 GB + models ≈ 14.7 GB) |
...
- **Model weights:** Qwen-Image-2.1 denoiser + VAE under the Qwen Research License; Qwen3-VL text encoder and vision projector under Apache-2.0 (downloaded by you at first run, never bundled).
