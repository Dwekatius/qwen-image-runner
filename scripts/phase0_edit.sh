#!/usr/bin/env bash
# Run one Phase 0 edit job with reference image(s).
# Usage: phase0_edit.sh <prefix> "<instruction>" <ref1.png> [ref2.png] ...
set -u
cd "$(dirname "$0")/.." || exit 1
LOG=${LOG:-logs/phase0-B.log}
PREFIX=$1; PROMPT=$2; shift 2

REFS=""
for f in "$@"; do
  B64=$(base64 -w0 "$f")
  [ -n "$REFS" ] && REFS="$REFS,"
  REFS="$REFS\"data:image/png;base64,$B64\""
done
...
RESP=$(curl -s -X POST http://127.0.0.1:1235/sdcpp/v1/img_gen -H "Content-Type: application/json" -d "{
  ...
  \"seed\": 7, \"batch_count\": 1,
  \"embed_image_metadata\": true,
  \"output_format\": \"png\",
  \"sample_params\": {\"scheduler\":\"discrete\",\"sample_method\":\"euler\",\"sample_steps\":40,\"guidance\":{\"txt_cfg\":6.0}}
}")
